...
void NLmeanswithopenmp(const uint8_t *imageSource, uint8_t *imageTarget,
                       uint16_t width, uint16_t height, double h,
                       uint8_t halfkernelsize, uint8_t halfsearch) {
  double h2 = (h * 2.0 + 1) * (h * 2.0 + 1) * 3;
  // double sigma2 = 5 * 5;
...
void NLmeanswithIntegralImageopenmp(const uint8_t *imageSource,
                                    uint8_t *imageTarget, uint16_t width,
                                    uint16_t height, double h,
                                    uint8_t halfkernelsize,
                                    uint8_t halfsearch) {
  double h2 = (h * 2.0 + 1) * (h * 2.0 + 1) * 3;
  // double sigma2 = 5 * 5;
...
#pragma omp parallel num_threads(8)
  {
    double *localsumR = new double[(width) * (height)];
    double *localsumG = new double[(width) * (height)];
    double *localsumB = new double[(width) * (height)];
    double *localweightsumR = new double[(width) * (height)];
    double *localweightsumG = new double[(width) * (height)];
    double *localweightsumB = new double[(width) * (height)];
    double *localweightmaxR = new double[(width) * (height)];
    double *localweightmaxG = new double[(width) * (height)];
    double *localweightmaxB = new double[(width) * (height)];
    double *localintegralR = new double[(width) * (height)];
    double *localintegralG = new double[(width) * (height)];
    double *localintegralB = new double[(width) * (height)];
...
#pragma omp for
    for (int wx = -(int)halfsearch; wx <= (int)halfsearch; wx++) {
      for (int wy = -(int)halfsearch; wy <= (int)halfsearch; wy++) {
        ...
        GenDifferIntegralImage(imageSource, localintegralR, width, height, wx,
                               wy, 2);
        GenDifferIntegralImage(imageSource, localintegralG, width, height, wx,
                               wy, 1);
        GenDifferIntegralImage(imageSource, localintegralB, width, height, wx,
                               wy, 0);
        ...
        for (int x = 0; x < width; ++x) {
          ...
          for (int y = 0; y < height; ++y) {
            ...
            distR = GetSumFromIntegralImg(localintegralR, width, height,
                                          (int16_t)x - (int16_t)halfkernelsize,
                                          (int16_t)y - (int16_t)halfkernelsize,
                                          (int16_t)x + (int16_t)halfkernelsize,
                                          (int16_t)y + (int16_t)halfkernelsize);
            distG = GetSumFromIntegralImg(localintegralG, width, height,
                                          (int16_t)x - (int16_t)halfkernelsize,
                                          (int16_t)y - (int16_t)halfkernelsize,
                                          (int16_t)x + (int16_t)halfkernelsize,
                                          (int16_t)y + (int16_t)halfkernelsize);
            distB = GetSumFromIntegralImg(localintegralB, width, height,
                                          (int16_t)x - (int16_t)halfkernelsize,
                                          (int16_t)y - (int16_t)halfkernelsize,
                                          (int16_t)x + (int16_t)halfkernelsize,
                                          (int16_t)y + (int16_t)halfkernelsize);
...
#if 1
            distR /=
                (2.0 * halfkernelsize + 1.0) * (2.0 * halfkernelsize + 1.0);
            distG /=
                (2.0 * halfkernelsize + 1.0) * (2.0 * halfkernelsize + 1.0);
            distB /=
                (2.0 * halfkernelsize + 1.0) * (2.0 * halfkernelsize + 1.0);
            ...
            localsumR[y * width + x] +=
                weightR * (*(imageSource + targetIndex + 2));
            localsumG[y * width + x] +=
                weightG * (*(imageSource + targetIndex + 1));
            localsumB[y * width + x] +=
                weightB * (*(imageSource + targetIndex + 0));

            localweightsumR[y * width + x] += weightR;
            localweightsumG[y * width + x] += weightG;
            localweightsumB[y * width + x] += weightB;

            localweightmaxR[y * width + x] =
                max(localweightmaxR[y * width + x], weightR);
            localweightmaxG[y * width + x] =
                max(localweightmaxG[y * width + x], weightG);
            localweightmaxB[y * width + x] =
                max(localweightmaxB[y * width + x], weightB);
          }
        }
      }
    }
#pragma omp critical
    for (int xy = 0; xy < width * height; ++xy) {
      sumR[xy] += localsumR[xy];
      sumG[xy] += localsumG[xy];
      sumB[xy] += localsumB[xy];
      weightsumR[xy] += localweightsumR[xy];
      weightsumG[xy] += localweightsumG[xy];
      weightsumB[xy] += localweightsumB[xy];
      weightmaxR[xy] = max(localweightmaxR[xy], weightmaxR[xy]);
      weightmaxG[xy] = max(localweightmaxG[xy], weightmaxG[xy]);
      weightmaxB[xy] = max(localweightmaxB[xy], weightmaxB[xy]);
    }
    delete[] localintegralR;
    delete[] localintegralG;
    delete[] localintegralB;

    delete[] localsumR;
    delete[] localsumG;
    delete[] localsumB;

    delete[] localweightsumR;
    delete[] localweightsumG;
    delete[] localweightsumB;

    delete[] localweightmaxR;
    delete[] localweightmaxG;
    delete[] localweightmaxB;
  }
